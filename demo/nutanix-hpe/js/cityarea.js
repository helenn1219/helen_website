        ...
        '南竿鄉', '北竿鄉', '莒光鄉', '東引鄉'
            ...
            $(document).ready(function(){
                for(var i=0;i<Object.keys(area_data).length;i++){
                    $("select[name=city]").append("<option value='"+Object.keys(area_data)[i]+"'>"+Object.keys(area_data)[i]+"</option>")
                }
            });

            $("select[name=city]").change(function(){
                var val=$("select[name=city]").val();
                $("select[name=area] option:not('.fixed-option')").remove()
                for(i=0;i<area_data[val].length;i++){
                    $("select[name=area]").append("<option value='"+area_data[val][i]+"'>"+area_data[val][i]+"</option>")
                }
                checkrent();
            });
            ...
            function checkrent(){
                ...
                // 非租案且縣市有值
                    custom.hide();
                    $("select[name=price] option:not('.fixed-option')").remove()
                    if(val=='臺北市'||val=='新北市'||val=='桃園市'||val=='臺中市'||val=='臺南市'||val=='高雄市'){ //非租案六縣市
                        for(var i=0;i<price['非租案六縣市'].length;i++){
                            $('select[name=price]').append("<option value='"+price['非租案六縣市'][i]+"'>"+price['非租案六縣市'][i]+"</option>")
                        }
                    }else{
                        for(var i=0;i<price['非租案'].length;i++){ //非租案
                            $('select[name=price]').append("<option value='"+price['非租案'][i]+"'>"+price['非租案'][i]+"</option>")
                        }
